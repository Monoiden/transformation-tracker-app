...
const STORAGE_KEY = "transformation-tracker-entries-v1";

export default function Page() {
  const [entry, setEntry] = useState<Entry>(emptyEntry);
  const [entries, setEntries] = useState<Entry[]>([]);
  const [activeTab, setActiveTab] = useState<"dashboard" | "daily" | "weekly">("dashboard");
  ...
  const proteinPct = Math.min(100, Math.round((Number(entry.protein || 0) / 190) * 100));
  const stepsPct = Math.min(100, Math.round((Number(entry.steps || 0) / 7000) * 100));
  const sleepPct = Math.min(100, Math.round((Number(entry.sleep || 0) / 7.5) * 100));
    ...
    <main className="page-shell">
      <section className="hero">
        <div>
          ...
          <p>
            Version 1 speichert Daten lokal auf deinem Gerät. Das ist der schnellste Start.
            Im nächsten Schritt kann Supabase für Cloud-Sync ergänzt werden.
          </p>
        </div>
        <div className="hero-badges">
          <span className="badge">PWA-Basis</span>
          <span className="badge">Mobile first</span>
          <span className="badge">Lokal speichernd</span>
        </div>
      </section>
      ...
      {activeTab === "dashboard" && (
        <section className="grid">
          ...
          <Card title="Letztes Protein" value={stats.latestProtein === "–" ? "–" : `${stats.latestProtein} g`} />
          <Card title="Gespeicherte Tage" value={`${stats.count}`} />
          <Card title="Status" value={entries.length ? "Daten vorhanden" : "Noch keine Daten"} />
        </section>
      )}
      ...
      {activeTab === "daily" && (
        <section className="panel">
          <div className="panel-header">
            ...
            <div className="target-hint">Zielwerte: 190 g Protein · 7000 Schritte · 7,5 h Schlaf</div>
          </div>

          <div className="form-grid">
            <Field label="Datum">
              <input type="date" value={entry.date} onChange={(e) => updateField("date", e.target.value)} />
            </Field>
            <Field label="Gewicht (kg)">
              <input value={entry.weight} onChange={(e) => updateField("weight", e.target.value)} placeholder="101.8" />
            </Field>
            <Field label="Kalorien">
              <input value={entry.calories} onChange={(e) => updateField("calories", e.target.value)} placeholder="2370" />
            </Field>
            <Field label="Protein (g)">
              <input value={entry.protein} onChange={(e) => updateField("protein", e.target.value)} placeholder="190" />
            </Field>
            <Field label="Kohlenhydrate (g)">
              <input value={entry.carbs} onChange={(e) => updateField("carbs", e.target.value)} placeholder="245" />
            </Field>
            <Field label="Fett (g)">
              <input value={entry.fat} onChange={(e) => updateField("fat", e.target.value)} placeholder="70" />
            </Field>
            <Field label="Schritte">
              <input value={entry.steps} onChange={(e) => updateField("steps", e.target.value)} placeholder="7000" />
            </Field>
            <Field label="Schlaf (h)">
              <input value={entry.sleep} onChange={(e) => updateField("sleep", e.target.value)} placeholder="7.5" />
            </Field>
            <Field label="Zigaretten">
              <input value={entry.cigarettes} onChange={(e) => updateField("cigarettes", e.target.value)} placeholder="0" />
            </Field>
            <Field label="Taillenumfang (cm)">
              <input value={entry.waist} onChange={(e) => updateField("waist", e.target.value)} placeholder="108" />
            </Field>
            <Field label="Ruhepuls">
              <input value={entry.restPulse} onChange={(e) => updateField("restPulse", e.target.value)} placeholder="60" />
            </Field>
            <div className="field">
            ...
            <Field label="Notizen">
              <textarea value={entry.notes} onChange={(e) => updateField("notes", e.target.value)} placeholder="Hunger, Performance, Stimmung, Besonderheiten ..." rows={4} />
            </Field>
          </div>

          <div className="progress-grid">
            <ProgressRow label="Protein" current={Number(entry.protein || 0)} target={190} pct={proteinPct} unit="g" />
            <ProgressRow label="Schritte" current={Number(entry.steps || 0)} target={7000} pct={stepsPct} unit="" />
            <ProgressRow label="Schlaf" current={Number(entry.sleep || 0)} target={7.5} pct={sleepPct} unit="h" />
          </div>
